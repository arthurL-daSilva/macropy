...
def CallRiot():
    try:
        ...
        os.startfile('C:\\Riot Games\\Riot Client\\RiotClientServices.exe') # <-- Your Riot Client Path Here
        return 0
...
def load_account_data():
    if not os.path.exists(data_file):
        messagebox.showwarning("Warning", "No file found, add at least one account to create it.")
        return
...
label_pass1 = Label(root, text="Password 1:")
...
label_pass2 = Label(root, text="Password 2:")
...
label_pass3 = Label(root, text="Password 3:")
...
label_pass4 = Label(root, text="Password 4:")
...
def acc1():
    if not entry_user1.get() or not entry_pass1.get():
        messagebox.showwarning("Warning", "Username or password of account 1 is not registered!")
        return

    label_name.config(text="Entering account 1...")
    value2 = CallRiot()
    value = Click(value2)
...
def acc2():
    if not entry_user2.get() or not entry_pass2.get():
        messagebox.showwarning("Warning", "Username or password of account 2 is not registered!")
        return

    label_name.config(text="Entering account 2...")
    value2 = CallRiot()
    value = Click(value2)
...
def acc3():
    if not entry_user3.get() or not entry_pass3.get():
        messagebox.showwarning("Warning", "Username or password of account 3 is not registered!")
        return

    label_name.config(text="Entering account 3...")
    value2 = CallRiot()
    value = Click(value2)
...
def acc4():
    if not entry_user4.get() or not entry_pass4.get():
        messagebox.showwarning("Warning", "Username or password of account 4 is not registered!")
        return

    label_name.config(text="Entering account 4...")
    value2 = CallRiot()
    value = Click(value2)
...
button1 = tk.Button(root, text="Enter account 1", command=acc1, relief='groove')
button1.place(x=300, y=50)

button2 = tk.Button(root, text="Enter account 2", command=acc2, relief='groove')
button2.place(x=300, y=110)

button3 = tk.Button(root, text="Enter account 3", command=acc3, relief='groove')
button3.place(x=300, y=170)

button4 = tk.Button(root, text="Enter account 4", command=acc4, relief='groove')
